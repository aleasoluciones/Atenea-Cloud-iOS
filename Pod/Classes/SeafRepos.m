...
- (void)renameLibrary: (SeafRepo *)repo newName:(NSString *)newName success:(void (^)(SeafDir *dir))success failure:(void (^)(SeafDir *dir, NSError *error))failure {
    NSString *requestUrl = [NSString stringWithFormat:API_URL"/repos/%@/?op=rename", repo.repoId];
    NSMutableString *form = [NSMutableString stringWithFormat:@"repo_name=%@", [newName escapedUrl]];
...
- (void)renameLibrary: (SeafRepo *)repo newName:(NSString *)newName
{
    [self renameLibrary:repo newName:newName success:nil failure:nil];
}


- (void) deleteLibrary:(void (^)(BOOL success))success{
    NSString *requestUrl = [NSString stringWithFormat:API_URL"/repos/%@/", self.repoId];
...
-(BOOL) canRead{
    return [[SeafPermissionManager sharedInstance] canRead:self];
}

-(BOOL) canWrite{
    //Shared repos cannot be writeables
    BOOL isSharedRepo = [self.repoType isEqualToString:SHARE_REPO];
    
    return  !isSharedRepo && [[SeafPermissionManager sharedInstance] canWrite:self];
}
...
- (id)initWithConnection:(SeafConnection *)aConnection
{
    Debug("SEARREPOS INITWITHCONNECTION %@",self.repoId);
