...
- (NSMutableDictionary*)convertTaskToDict:(id)task {
    ...
    if ([task isKindOfClass:[SeafFile class]]) {
        ...
        //Sync fields
        [Utils dict:dict setObject:ufile.syncId forKey:@"syncId"];
        [Utils dict:dict setObject:ufile.syncFileId forKey:@"syncFileId"];
...
-(void) loadDownloadingStoredTasksFrom:(SeafConnection *)connection{
    
    NSString *downloadKey = [self downloadStorageKey:connection.accountIdentifier];
    ...
    if (downloadTasks.allValues.count > 0) {
        for (NSDictionary *dict in downloadTasks.allValues) {
            SeafFile *file = [[SeafFile alloc] initWithConnection:connection oid:[dict objectForKey:@"oid"] repoId:[dict objectForKey:@"repoId"] name:[dict objectForKey:@"name"] path:[dict objectForKey:@"path"] mtime:[[dict objectForKey:@"mtime"] longLongValue] size:[[dict objectForKey:@"size"] longLongValue]];
...
- (void)startLastTimeUnfinshTaskWithConnection:(SeafConnection *)conn {
    ...
    [self loadDownloadingStoredTasksFrom:conn];
    
    [self loadUploadingStoredTasksFrom:conn];
