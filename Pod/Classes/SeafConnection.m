...
@property SeafSyncronizer *syncronizer;
...
- (id)initWithUrl:(NSString *)url cacheProvider:(id<SeafCacheProvider>)cacheProvider username:(NSString *)username
{
    ...
    _syncronizer = [SeafSyncronizer sharedInstanceFor:self];
...
- (double)getMaxPacketSizeInBytes
{
    return  self.getPlan.uploadMaxPacketSizeInBytes;
}

- (double)getMaxBandwidthDelayInSeconds
{
    return  self.getPlan.uploadDelayInSeconds;
}
...
- (void)downloadDir:(SeafDir *)dir
{
    [dir loadContentSuccess:^(SeafDir *dir) {
        __block NSInteger failedFileCount = 0;
        ...
        for (SeafBase *item in dir.items) {
            if ([item isKindOfClass:[SeafFile class]]) {
                ...
                SeafCompositeFilter *uploadFilter = [SeafFileFilterBuilder getFilterForPreview:self repoId:item.repoId];
                ...
                if (![uploadFilter meetsCondition:adapterFile firstFailedFilter:&failedFilter]) {
                    failedFileCount++;
                    Debug("Error download file: %@", failedFilter);
                }else{
        ...
        if (failedFileCount > 0) {
            [SeafAlertChangePlan showAlert: NSLocalizedString(@"Download error some file. Do you want to change the plan?", @"Seafile")];
        }
    } failure:^(SeafDir *dir, NSError *error) {
