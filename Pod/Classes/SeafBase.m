...
- (void)generateShareLink:(id<SeafShareDelegate>)dg password:(NSString *)password expire_days:(NSString *)expire_days {
    ...
    if (password) {
        form = [form stringByAppendingString:[NSString stringWithFormat:@"&password=%@", password]];
    }
    if (expire_days) {
        form = [form stringByAppendingString:[NSString stringWithFormat:@"&expire_days=%@", expire_days]];
    }
        ...
        if (JSON && [JSON isKindOfClass:[NSArray class]]) {
            ...
            if (list.count > 0) {
                ...
                if (dict && [dict objectForKey:@"link"]) {
                    ...
                    [self deleteShareLink:token completionHandler:^(BOOL success) {
                        if (success) {
                            NSLog(@"Enlace compartido eliminado: %@", link);
                        } else {
                            NSLog(@"Error al eliminar el enlace compartido: %@", link);
                        }
                   }];
...
- (void)deleteShareLink:(NSString *)token completionHandler:(void (^)(BOOL success))completionHandler {
    NSString *url = [NSString stringWithFormat:@"%@/share-links/%@/", API_URL_V21, token];
    [connection sendDelete:url success:^(NSURLRequest * _Nonnull request, NSHTTPURLResponse * _Nonnull response, id  _Nonnull JSON) {
        completionHandler(YES);
    } failure:^(NSURLRequest * _Nonnull request, NSHTTPURLResponse * _Nullable response, id  _Nullable JSON, NSError * _Nullable error) {
        NSLog(@"Detalles: %@", error.localizedDescription);
...
- (void)generateShareLinkPass:(id<SeafShareDelegate>)dg password:(NSString *)password
{
   [self getShareLink:^(BOOL result, NSString *link) {
       if (result && link.length > 0) {
           self->_shareLink = link;
           [self generateShareLink:dg password:password expire_days:nil];
       } else {
           [self generateShareLink:dg password:password expire_days:nil];
       }
   }];
