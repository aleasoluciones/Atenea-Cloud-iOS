...
-(BOOL) canWrite{
    ...
    //If repo is no editable, all items are not editables
    SeafRepo *repo = [connection getRepo:self.repoId];
    if(repo && [repo.repoType isEqualToString:SHARE_REPO]){
        return NO;
    }
    
    
    
    return [[SeafPermissionManager sharedInstance] canWrite:self];
}
