...
- (BOOL)editable
{
    return [self canWrite] && [[connection getRepo:self.repoId] editable] && [self.mime hasPrefix:@"text/"];
}
...
- (void)update:(id<SeafFileUpdateDelegate>)dg
{
    ...
    if (!self.ufile) {
        ...
        self.ufile.removeSourceAfterUpload = YES;
...
-(NSString *) permissions{
    return [[connection getRepo:self.repoId] permissions];
}
...
-(BOOL) canWrite{
    SeafRepo *parentRepo = [connection getRepo:self.repoId];
    
    return [parentRepo canWrite] && [[SeafPermissionManager sharedInstance] canWrite:self];
}
