...
@synthesize removeSourceAfterUpload = _removeSourceAfterUpload;
@synthesize syncId;
@synthesize onlyWifi;
...
- (void)cleanup
{
    ...
    if(self.removeSourceAfterUpload == FALSE) {
        Debug("Remove source file CANCELLED: %@", self.lpath);
        return;
    }
...
- (void)uploadByFile:(SeafConnection *)connection url:(NSString *)surl path:(NSString *)uploadpath update:(BOOL)update
{
    NSMutableURLRequest *request = [[SeafConnection requestSerializer] multipartFormRequestWithMethod:@"POST" URLString:surl parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        [formData throttleBandwidthWithPacketSize:[connection getMaxPacketSizeInBytes] delay:[connection getMaxBandwidthDelayInSeconds]];
