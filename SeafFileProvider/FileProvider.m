...
- (void)startProvidingItemAtURL:(NSURL *)url completionHandler:(void (^)(NSError *))completionHandler
{
    ...
    //MARK: Plan filter
    SeafFileAdapter *adapterFile = [[SeafFileAdapter alloc] initWithSeafFile:file];
    SeafCompositeFilter *uploadFilter = [SeafFileFilterBuilder getFilterForPreview:file->connection repoId:file.repoId];
    id<SeafFileFilterStrategy> filterFailed;
    if(FALSE == [uploadFilter meetsCondition:adapterFile firstFailedFilter:&filterFailed] ){
        completionHandler([NSError fileProvierErrorFeatureUnsupportedPlanFilter:filterFailed]);
        return;
    }
          ...
          completionHandler:(void (^)(NSFileProviderItem _Nullable importedDocumentItem, NSError * _Nullable error))completionHandler
{
    ...
    //MARK: Plan UPLOAD filter
    SeafFile *seafile = [[SeafFile alloc] initWithConnection:item.conn oid:nil repoId:item.repoId name:item.name path:fileURL.absoluteString mtime:0 size:0];
    [seafile setMpath:localURL.path];
  
    
    //TODO: DEBEMOS PINTAR EL ERROR. AL TRABAJAR CON LA INTERFACE NO TENEMOS FIRSTFAILEDFILTER.
    //DEBERÍAMOS USAR EL COMPOSITEFILTER
   
    SeafFileAdapter *adapterFile = [[SeafFileAdapter alloc] initWithSeafFile:seafile];
    SeafCompositeFilter *uploadFilter = [SeafFileFilterBuilder getFilterForUpload:item.conn];
    id<SeafFileFilterStrategy> filterFailed;
    

    if(FALSE == [uploadFilter meetsCondition:adapterFile firstFailedFilter:&filterFailed] ){
        completionHandler(nil, [NSError fileProvierErrorFeatureUnsupportedPlanFilter:filterFailed]);
        return;
    }
               ...
               completionHandler:(void (^)(NSFileProviderItem _Nullable renamedItem, NSError * _Nullable error))completionHandler;
{
    ...
    //Renaming extension? Not allowed
    NSString *oldExtension = [[item.name pathExtension] lowercaseString];
    NSString *newExtension = [[itemName pathExtension] lowercaseString];
    if (![oldExtension isEqualToString:newExtension]) {
          completionHandler(nil, [NSError fileProviderErrorRenameFileExtension]);
          return;
    }
