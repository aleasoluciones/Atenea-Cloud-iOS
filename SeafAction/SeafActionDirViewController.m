...
@property (nonatomic, strong) SeafTrashChecker *trashChecker;
...
- (id)initWithSeafDir:(SeafDir *)directory file:(SeafUploadFile *)ufile
{
    if (self = [super init]) {
        ...
        _ufile = ufile;
        _trashChecker = [[SeafTrashChecker alloc] initWithProvider:[[SeafRecoveryFileFolderProvider alloc] initWithConnection:directory.getConnection andRepository:self.directory.repoId andPath:self.directory.path]];
...
- (void)createFolder {
    ...
    [self popupInputView:NSLocalizedString(@"New Folder", @"Seafile") placeholder:NSLocalizedString(@"New folder name", @"Seafile") secure:false handler:^(NSString *input) {
        ...
        NSString *newPath = [self.directory.path stringByAppendingPathComponent:input];
        
        [self.trashChecker existsInTrash:newPath callback:^(BOOL exists) {
            
            if(exists == FALSE){
                //Its ok. We send input instead of newPath
                [self performCreateFolder:input];
            }
            else{
                [self askForOverwriteTrash:input];
            }
...
- (void) askForOverwriteTrash:(NSString *) input{
    
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Existe un elemento con el mismo nombre en la papelera.\n¿Quiere continuar?", @"Seafile") message:nil preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:STR_CANCEL style:UIAlertActionStyleCancel handler:nil];
    
    UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:STR_CANCEL style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
        [self performCreateFolder:input];
    }];
...
- (void) performCreateFolder:(NSString *) newFolderName{
    
    [self.directory mkdir:newFolderName success:^(SeafDir *dir) {
        [self dismissLoadingView];
