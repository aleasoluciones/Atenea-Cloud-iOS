...
-(void) startSyncronizerFor:(SeafConnection *) connection{

        //Migrate legacy camera uploads if needed
        [SeafSyncLegacyMigrator migrate:connection];
...
-(void) startSyncronizers{
    
    for (SeafConnection *conn in SeafGlobal.sharedObject.conns) {
        
        [[SeafDataTaskManager sharedObject] loadUploadingStoredTasksFrom:conn];
        
        //Migrate legacy camera uploads if needed
        [SeafSyncLegacyMigrator migrate:conn];
...
- (void)delayedInit
{
    ...
    //Commented.Set individual syncronizer on "enterAccount" method
    //[self startSyncronizers];
...
-(void) registerBackgroundTasks{
    if (@available(iOS 13.0, *)) {
        ...
        SeafBackgroundTasksManager *taskManager = [SeafBackgroundTasksManager sharedInstance];
        [[taskManager getTasks] enumerateObjectsUsingBlock:^(id<SeafBackgroundTaskProtocol>  seafTask, NSUInteger idx, BOOL * _Nonnull stop) {
            [BGTaskScheduler.sharedScheduler registerForTaskWithIdentifier:[seafTask identifier] usingQueue:nil launchHandler:^(__kindof BGTask * _Nonnull task) {

                task.expirationHandler = ^{
                    [taskManager submitBackgroundTask:seafTask];
                };
                
                [seafTask onComplete:^(BOOL success) {
                    [task setTaskCompletedWithSuccess:success];
                    [taskManager submitBackgroundTask:seafTask];
                    NSLog(@"#BACKGROUND_TASKS:: SCHEDULED:: %@",seafTask.identifier);
                }];
               
                NSLog(@"#BACKGROUND_TASKS:: RUNNING:: %@",seafTask.identifier);
                [seafTask run];
            }];
...
-(void) requestBackgroundTasks{
    if (@available(iOS 13.0, *)) {
        SeafBackgroundTasksManager *taskManager = [SeafBackgroundTasksManager sharedInstance];
        [taskManager submitBackgroundAllTasks];
    }
}
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    [_global migrate];
    [SeafUIBridge initializeWith:self.window];
    [self initTabController];
    
    [[UITabBar appearance] setTintColor:[UIColor colorWithRed:0.0/255.0 green:173.0/255.0 blue:239.0/255.0 alpha:1.0]];
...
-(void) startLocationObserver{
    SeafSyncLocationObserver *locationObserver = [SeafSyncLocationObserver sharedInstance];
    [locationObserver start:nil];
}
...
- (void)photoLibraryDidChange:(PHChange *)changeInstance
{
    ...
    NSLog(@"photoLibraryDidChange en SEAFAPPDELEGATE");
