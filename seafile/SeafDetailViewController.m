...
- (void)updateNavigation
{
    ...
    //If no writeable, remove edit&delete
    if(!self.preViewItem.canWrite){
        [array removeObject:self.editItem];
        [array removeObject:self.deleteItem];
    }
...
- (void)viewDidLoad
{
    ...
    self.exportItem = [self getBarItemAutoSize:@"export2-blue" action:@selector(export:)];
...
- (IBAction)delete:(id)sender {
    ...
    if(![self.preViewItem canWrite]){
        [self alertWithTitle:NSLocalizedString(@"This is a Read-Only element", @"Seafile")];
        return;
    }
