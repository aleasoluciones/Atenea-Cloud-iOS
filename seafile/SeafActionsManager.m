...
+ (void)entryAction:(SeafBase *)entry inEncryptedRepo:(BOOL)inEncryptedRepo inTargetVC:(UIViewController *)targetVC fromView:(UIView *)view actionBlock:(ActionType)block {
    ...
    if ([entry isKindOfClass:[SeafRepo class]]) {
        ...
        } if (![@"srepo" isEqualToString:repo.repoType]) {
            [titles addObject:S_LIB_RENAME];
            [titles addObject:S_DELETE_LIB];
        }
        ...
        if(!file.canWrite){
            if (file.mpath)
                titles = [NSMutableArray arrayWithObjects:star, S_DOWNLOAD, nil];
            else
                titles = [NSMutableArray arrayWithObjects:star, S_REDOWNLOAD, nil];
        }
    ...
    if (![entry isKindOfClass:[SeafRepo class]] && !inEncryptedRepo) {
        [titles addObjectsFromArray:@[S_SHARE_EMAIL, S_SHARE_PASS ,S_SHARE_LINK]];
    }
...
+ (void)directoryAction:(SeafDir*)directory photos:(NSArray *)photos inTargetVC:(UIViewController *)targetVC fromItem:(UIBarButtonItem *)item actionBlock:(ActionType)block {
    ...
    if ([directory isKindOfClass:[SeafRepos class]]) {
        titles = [NSMutableArray arrayWithObjects:S_MKLIB,S_MKDELETLIB,S_SORT_NAME, S_SORT_MTIME, S_LIB_RENAME, nil];
    } else if (directory.editable) {
        titles = [NSMutableArray arrayWithObjects:S_EDIT, S_NEWFILE, S_MKDIR,S_MKDELETFILE,S_SORT_NAME, S_SORT_MTIME, S_PHOTOS_ALBUM, nil];
    ...
    //No writeable => overwrite actions
