...
@interface SeafFileViewController ()<QBImagePickerControllerDelegate, SeafUploadDelegate, SeafDirDelegate, SeafShareDelegate, MFMailComposeViewControllerDelegate, SWTableViewCellDelegate, MWPhotoBrowserDelegate,UINavigationControllerDelegate, UIImagePickerControllerDelegate>
...
- (void)setConnection:(SeafConnection *)conn
{
    ...
    [[SeafSyncronizer sharedInstanceFor:self.connection] notifyErrorsIfNeeded];
    
    /* [conn deletedRepositories:^(NSArray<SeafDeletedRepoItem *> * _Nonnull items) {
     id deleted = items;
     }];*/
}
...
- (void)addPhotos:(id)sender
{
    UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:nil
                                                             delegate:self
                                                    cancelButtonTitle:NSLocalizedString(@"Cancel", @"Seafile")
                                               destructiveButtonTitle:nil
                                                    otherButtonTitles:NSLocalizedString(@"Choose from Library", @"Seafile"), NSLocalizedString(@"Take Photo", @"Seafile"), nil];
    
    actionSheet.actionSheetStyle = UIActionSheetStyleDefault;
    [actionSheet showFromBarButtonItem:self.photoItem animated:YES];
}
...
- (void)openCamera {
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        [self alertWithTitle:NSLocalizedString(@"Camera not available", @"Seafile") message:NSLocalizedString(@"Sorry, but it seems that your device does not have a camera.", @"Seafile")];
    }
}
...
- (void)initNavigationItems:(SeafDir *)directory
{
    if (![directory isKindOfClass:[SeafRepos class]] && directory.editable) {
        self.photoItem = [self getBarItem:@"plus_blue" action:@selector(addPhotos:)size:20];
        ...
        self.editItem = [self getBarItemAutoSize:@"ellipsis2_blue" action:@selector(editSheet:)];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (cached || waiting || isDownloading) {
            ...
            if (isDownloading) {
                ...
                NSString *downloadImageNmae = waiting ? @"download_waiting" : @"download_finished_blue";
...
- (void)popupMkdirView
{
    ...
    [self popupInputView:S_MKDIR placeholder:NSLocalizedString(@"New folder name", @"Seafile") secure:false handler:^(NSString *input) {
        ...
        [self createItemWithTrashCheck:input onConfirmOverwrite:^{
...
- (void)popupRenameViewWithExt:(NSString *)oldName withExtension:(NSString *)extension
{
    self.state = STATE_RENAME;
    NSString *oldNameWithExtension = [oldName stringByAppendingFormat:@".%@", extension];
    ...
    [self popupInputView:S_RENAME placeholder:oldName inputs:oldName secure:false handler:^(NSString *input) {
        ...
        NSString *newNameWithExtension = [input stringByAppendingFormat:@".%@", extension];
        
        [_directory renameEntry:oldNameWithExtension newName:newNameWithExtension];
        [SVProgressHUD showWithStatus:NSLocalizedString(@"Renaming file ...", @"Seafile")];
    }];
}

- (void)popupRenameLibView:(SeafRepo *)repoToRename
{
    Debug(@"Estoy en la ventana modal DE EDICION DE LIBRERIAS");
    Debug(@"%@", repoToRename.name);
    
    self.state = STATE_LIB_RENAME;
    [self popupInputView:S_LIB_RENAME placeholder:repoToRename.name inputs:repoToRename.name secure:false handler:^(NSString *input) {
        if ([input isEqualToString:repoToRename.name]) {
            return;
        }
        if (!input || input.length == 0) {
            [self alertWithTitle:NSLocalizedString(@"File name must not be empty", @"Seafile")];
        ...
        SeafRepo *editRepo = (SeafRepo*)repoToRename;
        [editRepo renameLibrary:editRepo newName:(NSString *)input];
        [SVProgressHUD showWithStatus:NSLocalizedString(@"Renaming file ...", @"Seafile")];
    }];
}

- (void)popupDeleteLibView:(SeafRepo *)repoToDelete
{
    
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Confirm", @"Seafile")  message:NSLocalizedString(@"Remove this library?", @"Seafile")  preferredStyle:UIAlertControllerStyleAlert];
    ...
    UIAlertAction *continueAction = [UIAlertAction actionWithTitle:@"Remove"
        ...
        [self performDelete:repoToDelete];
...
-(void) performDelete:(SeafRepo *)repoToDelete{
    
    [SVProgressHUD showWithStatus:NSLocalizedString(@"Removing library ...", @"Seafile")];
    
    __weak typeof(self) weakSelf = self;
    
    [repoToDelete deleteLibrary:^(BOOL success) {
        [SVProgressHUD dismiss];
        [weakSelf.directory loadContent:true];
    }];
}
...
- (void)handleSelection:(NSObject *)entry indexPath:(NSIndexPath *)indexPath tableView:(UITableView *)tableView {
    ...
    Debug("Select i %@", [_curEntry valueForKey:@"name"]);
    if (!_curEntry) {
    ...
    if ([_curEntry conformsToProtocol:@protocol(SeafPreView)]) {
        ...
        if (self.detailViewController.state == PREVIEW_QL_MODAL) { // Use fullscreen preview for doc, xls, etc.
            ...
            if (IsIpad()) {
                // Use fullscreen on iPad, QLPreviewController's navigation bar has action items
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSObject *entry = [self getDentrybyIndexPath:indexPath tableView:tableView];
    SeafCompositeFilter *uploadFilter = [SeafFileFilterBuilder getFilterForPreview:_connection repoId:_directory.repoId];
    ...
    if ([entry isKindOfClass:[SeafFile class]]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            SeafFile *seafFile = (SeafFile *)entry;
            SeafFileAdapter *adapterFile = [[SeafFileAdapter alloc] initWithSeafFile:seafFile];
            id<SeafFileFilterStrategy> failedFilter;
            
            if (![uploadFilter meetsCondition:adapterFile firstFailedFilter:&failedFilter]) {
                [SeafAlertChangePlan showAlertWithFilterStrategy:failedFilter];
                // Detener el spinner en el hilo principal
                [spinner stopAnimating];
                return;
                
            }
            [spinner stopAnimating];
            [self handleSelection:entry indexPath:indexPath tableView:tableView];
        });
    ...
    if (entry == _directory) {
        ...
        switch (self.state) {
            ...
            case STATE_MKLIB:
                ...
                break;
            case STATE_LIB_RENAME:{
                [SVProgressHUD showErrorWithStatus:NSLocalizedString(@"Failed to rename library", @"Seafile")];
                NSString *oldName = [(SeafRepo *)_curEntry name];
                [self performSelector:@selector(popupRenameLibView:) withObject:oldName afterDelay:1.0];
                break;
            }
            case STATE_LIB_DELETE:{
                [SVProgressHUD showErrorWithStatus:NSLocalizedString(@"Failed to delete library", @"Seafile")];
                [self performSelector:@selector(deleteLibrary) withObject:nil afterDelay:1.0];
                break;
            }
            default:
                break;
...
- (void)renameEntry:(SeafBase *)obj
{
    ...
    NSString *fileName = [obj.name stringByDeletingPathExtension];
    NSString *fileExtension = [obj.name pathExtension];
    
    if (![fileName isEqualToString:obj.name]) {
        [self popupRenameViewWithExt:fileName withExtension:fileExtension];
    } else {
        [self popupRenameView:fileName];
    }
}

- (void)renameLib:(SeafRepo *)repoToRename
{
    _curEntry = repoToRename;
    [self popupRenameLibView:repoToRename];
}

- (void)deleteLibrary:(SeafRepo *)repoToDelete
{
    _curEntry = repoToDelete;
    [self popupDeleteLibView:repoToDelete];
}
...
- (void)handleAction:(NSString *)title
{
    ...
    Debug("TITULO: %@", title);
    Debug("SELECTED CELL: %@", _selectedCell);
    ...
    if ([S_NEWFILE isEqualToString:title]) {
    ...
    } else if ([S_SHARE_PASS isEqualToString:title]) {
        self.state = STATE_SHARE_LINK_PASS;
        [self showShareLinkPasswordAlert];
        ...
        Debug(@"create lib");
        [self popupMklibView];
    } else if ([S_LIB_RENAME isEqualToString:title]) {
        SeafRepo *repo = (SeafRepo *)[self getDentrybyIndexPath:_selectedindex tableView:self.tableView];
        
        [self renameLib:repo];
    }else if ([S_MKDELETLIB isEqualToString:title]) {
        [self popupDeletedLibraries];
    }else if([S_MKDELETFILE isEqualToString:title]){
        Debug(@"deleted files");
        [self popupDeletedFiles];
    }else if([S_DELETE_LIB isEqualToString:title]){
        SeafBase *entry = (SeafBase *)[self getDentrybyIndexPath:_selectedindex tableView:self.tableView];
        Debug(@"%@", entry);
        [self deleteLibrary:(SeafRepo *)entry];
    }
}


- (void)showShareLinkPasswordAlert {
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"file_action_share_encrypt", @"Seafile") message:NSLocalizedString(@"password_sharing_rules", @"Seafile") preferredStyle:UIAlertControllerStyleAlert];

    [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.placeholder = NSLocalizedString(@"Enter password", @"Seafile");
        textField.secureTextEntry = YES;
    }];

    __block UITextField *confirmPasswordField = nil;
    [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.placeholder = NSLocalizedString(@"Confirm password", @"Seafile");
        textField.secureTextEntry = YES;
        confirmPasswordField = textField;
    }];

    [self addPasswordVisibilityButtonToTextField:alertController.textFields.firstObject];
    [self addPasswordVisibilityButtonToTextField:alertController.textFields[1]];

    [alertController addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Cancel", @"Seafile") style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
    }]];

    [alertController addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"file_action_share_link", @"Seafile") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [alertController dismissViewControllerAnimated:YES completion:^{
            if (password.length == 0 || confirmPassword.length == 0) {
                [self showAlertWithMessage:NSLocalizedString(@"password_not_meet_requirements", @"Seafile")];
            } else if (![password isEqualToString:confirmPassword]) {
                [self showAlertWithMessage:NSLocalizedString(@"password_not_meet_requirements", @"Seafile")];
            } else if (![self isPasswordValid:password] || ![self isPasswordValid:confirmPassword]) {
                [self showAlertWithMessage:NSLocalizedString(@"password_not_meet_requirements", @"Seafile")];
            } else if (![self isPasswordStrong:password]) {
                [self showAlertWithMessage:NSLocalizedString(@"password_not_meet_requirements", @"Seafile")];
            } else {
                SeafBase *entry = (SeafBase *)[self getDentrybyIndexPath:_selectedindex tableView:self.tableView];
                [self generateShareLinkWithPassword:password expiredDays:nil forEntry:entry];
            }
        }];
    }]];
...
- (void)addPasswordVisibilityButtonToTextField:(UITextField *)textField {
    UIButton *visibilityButton = [UIButton buttonWithType:UIButtonTypeSystem];
       visibilityButton.frame = CGRectMake(0, 0, 10, 10);
    if (@available(iOS 13.0, *)) {
        UIImage *eyeImage = [[UIImage systemImageNamed:@"eye"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        
        [visibilityButton setImage:eyeImage forState:UIControlStateNormal];
    } else {
    }
       visibilityButton.tintColor = [UIColor grayColor];
       [visibilityButton addTarget:self action:@selector(togglePasswordVisibilityForTextField:) forControlEvents:UIControlEventTouchUpInside];
       textField.rightView = visibilityButton;
       textField.rightViewMode = UITextFieldViewModeAlways;
}

- (void)togglePasswordVisibilityForTextField:(UIButton *)button {
...
- (void)generateShareLinkWithPassword:(NSString *)password expiredDays:(NSString *)expiredDays forEntry:(SeafBase *)entry {
    [SVProgressHUD showWithStatus:NSLocalizedString(@"Generate share link ...", @"Seafile")];
    [entry generateShareLinkPass:self password:password];
}
...
- (BOOL)isPasswordStrong:(NSString *)password {
    ...
    if ([password rangeOfCharacterFromSet:[NSCharacterSet lowercaseLetterCharacterSet]].location != NSNotFound) {
        conditionsMet++;
    }
    ...
    if ([password rangeOfCharacterFromSet:symbolCharacterSet].location != NSNotFound) {
        conditionsMet++;
    }
...
- (BOOL)isFirstLetterUppercase:(NSString *)password {
    if (password.length > 0) {
        unichar firstChar = [password characterAtIndex:0];
        return [[NSCharacterSet uppercaseLetterCharacterSet] characterIsMember:firstChar];
    }
    return NO;
}
...
- (void)uploadPickedAssetsIdentifier:(NSArray *)identifiers overwrite:(BOOL)overwrite {
    ...
    [self checkItemsInTrash:files onConfirmOverwrite:^{
        [self permformUploadPickedAssetsIdentifier: files];
    }];
}


-(void) permformUploadPickedAssetsIdentifier:(NSArray<SeafUploadFile *> *) files{
    
    [files enumerateObjectsUsingBlock:^(SeafUploadFile * _Nonnull file, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)qb_imagePickerController:(QBImagePickerController *)imagePickerController didFinishPickingAssets:(NSArray *)assets {
    ...
    SeafCompositeFilter *uploadFilter = [SeafFileFilterBuilder getFilterForUpload:_connection];
    for (PHAsset *asset in assets) {
        SeafSyncAssetItem *photoAsset = [[SeafSyncAssetItem alloc]initWithPHAsset:asset];
        SeafAssetFileAdapter *adapterFile = [[SeafAssetFileAdapter alloc] initWithPhotoAsset:photoAsset];
        id<SeafFileFilterStrategy> failedFilter;
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<UIImagePickerControllerInfoKey,id> *)info {
    ...
    __block NSString *assetIdentifier;
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        ...
        if (success) {
            PHAsset *capturedAsset = [PHAsset fetchAssetsWithLocalIdentifiers:@[assetIdentifier] options:nil].firstObject;
            [self processCapturedPhoto:capturedAsset];
        } else {
            NSLog(@"Error %@", error);
        }
    }];
...
- (void)processCapturedPhoto:(PHAsset *)capturedAsset {

    SeafCompositeFilter *uploadFilter = [SeafFileFilterBuilder getFilterForUpload:_connection];
    SeafSyncAssetItem *photoAsset = [[SeafSyncAssetItem alloc] initWithPHAsset:capturedAsset];
    SeafAssetFileAdapter *adapterFile = [[SeafAssetFileAdapter alloc] initWithPhotoAsset:photoAsset];
    id<SeafFileFilterStrategy> failedFilter;
    ...
    [self uploadPickedAssetsIdentifier:@[capturedAsset.localIdentifier] overwrite:false];
}
...
- (void)generateSharelink:(SeafBase *)entry WithResult:(BOOL)success
{
    ...
    }else if(self.state == STATE_SHARE_LINK_PASS){
        [self shareLinkPass:entry];
    }
}

- (void)shareLinkPass:(SeafBase *)entry {
    NSString *shareLink = entry.shareLink;
    if (shareLink) {
        NSArray *itemsToShare = @[shareLink];
        UIActivityViewController *activityViewController = [[UIActivityViewController alloc] initWithActivityItems:itemsToShare applicationActivities:nil];
        activityViewController.title = NSLocalizedString(@"Compartir enlace", @"Seafile");
...
//MARK: SeafRecoverySelectorViewControllerDelegate
-(void) onRecovery{
...
-(void) createItemWithTrashCheck:(NSString *) itemName onConfirmOverwrite:(void (^ _Nullable)(void))onConfirm{
    ...
    NSString *newCompletePath = [self.directory.path stringByAppendingPathComponent:itemName];
    
    [trashChecker existsInTrash:newCompletePath callback:^(BOOL exists) {
        if(exists == FALSE){
            onConfirm();
        }
        else{
            [SVProgressHUD dismiss];
            [self askForOverwriteTrash:itemName onConfirmOverwrite:(void (^ _Nullable)(void))onConfirm];
        }
    }];
...
- (void) askForOverwriteTrash:(NSString *) input onConfirmOverwrite:(void (^ _Nullable)(void))onConfirm{
    
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"There is an item with the same name in the trash.\nDo you want to proceed?", @"Seafile")  message:nil  preferredStyle:UIAlertControllerStyleAlert];
